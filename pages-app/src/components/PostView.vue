...
<b-container>
  <b-row>
    ...
    <b-col>{{post.datetime | unixToDate}}</b-col>
  </b-row>
...
export default {
  ...
  mounted: async function() {
    const result = await axios.get(`${process.env.VUE_APP_ARTICLE_API}/posts/${this.id}`)
    this.post = result.data.data as PostType
  },
  filters: {
    unixToDate(date: number) {
